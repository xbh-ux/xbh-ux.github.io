...
_PreferenceRank = tuple[int, int, int]


def _subtitle_score(name: str, subtitle: str | None = None) -> int:
    """字幕优先级：简体优先；没有简体时再接受双语/简繁/内封/繁体。"""
    text = f"{name} {subtitle or ''}".lower()
    simplified_terms = (
        "简体",
        "简中",
        "简日",
    ...
    bilingual_terms = ("双语", "日双", "英双", "内封", "内嵌", "外挂")
    no_sub_terms = ("raw", "无字幕", "no sub", "nosub")

    has_simplified = any(term in text for term in simplified_terms)
    has_traditional = any(term in text for term in traditional_terms)
    has_bilingual = any(term in text for term in bilingual_terms)
    has_no_sub = any(term in text for term in no_sub_terms)

    if has_simplified and not has_traditional and not has_bilingual:
        return 60
    if has_simplified and has_bilingual and not has_traditional:
        return 55
    if has_simplified:
        return 50
    if has_traditional and has_bilingual:
        return 35
    if has_traditional:
        return 30
    if has_bilingual:
...
class RSSEngine:
    ...
    def _select_preference_skips(
        ...
        """按字幕/发布组/分辨率偏好去重：同一集只保留最匹配偏好的版本。

        默认所有番剧都启用字幕优先级：简体 > 简体双语/简繁/内封 >
        繁体/繁体内封 > 其他内封/外挂 > RAW/无字幕。设置了
        ``preferred_group`` 或 ``preferred_resolution`` 的番剧会在字幕优先级
        相同的情况下继续按这些偏好细分。
        ...
        for bangumi_id, torrents in existing_downloaded.items():
            ...
            for torrent in torrents:
                ...
                rank = (
                    _subtitle_score(torrent.name, getattr(release, "subtitle", None)),
    ...
    async def _refresh_rss(
        ...
        # Every bangumi uses per-episode subtitle preference dedup. Optional
        # release-group/resolution preferences are still honoured as tie-breakers.
        preference_bangumi = {
            b.id: b
            for b in bangumi_list
            if b.id is not None and not b.deleted
        }
        existing_downloaded: dict[int, list[Torrent]] = {}
