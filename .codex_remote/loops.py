...
async def calendar_tick() -> None:
    """Skip upstream Bangumi.tv calendar refresh.

    This deployment infers weekdays from Mikan RSS publish dates via the
    weekday-linker timer. The upstream api.bgm.tv calendar endpoint times out
    from this server and only adds noisy warnings/errors to the Web UI log.
    """
    logger.debug("Skip built-in Bangumi.tv calendar refresh.")
